...
> Status: implementation complete; formal GPU runs not yet executed. Every result cell below intentionally remains pending until it is generated by the included evaluator.
...
No result is hard-coded. The final table must be filled only from `results/evaluation/*.json` and the two branch `run_metadata.json` files.
...
Commit the exact code/config state before starting. Then run only these three main training jobs, in order:

```bash
bash scripts/prepare_data.sh
bash scripts/evaluate_base.sh       # pre-evaluation gate
bash scripts/train_sft.sh            # run 1
bash scripts/train_grpo.sh           # run 2
bash scripts/train_continued_sft.sh  # run 3; reads GRPO's measured budget
...
The failure-analysis sheet is deliberately not auto-labeled. A human must inspect and categorize examples as arithmetic error, wrong strategy, correct reasoning/wrong extraction, incomplete reasoning, overlong reasoning, formatting error, RL-specific degeneration, or successful corrected reasoning.
...
This is a small-model, single-domain, one-primary-seed study. GPU time is practical but not hardware-independent compute. The continued-SFT branch consumes labeled solutions while GRPO consumes prompts and online generations, so the comparison answers a resource-allocation question rather than equating supervision sources. Exact-match verification measures answer correctness, not reasoning validity. A second seed is required when the headline difference is small.
